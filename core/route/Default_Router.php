...
// core/route/Default_Router.php
 class SqueHub_Default_Router
 {
     ...
     private $authCheckCallbacks = [];

     public function setAuthCheckCallback($routeType, $callback)
     {
         $this->authCheckCallbacks[$routeType] = $callback;
     }
     ...
     public function route()
     {
         ...
         if (empty($matches)) {
          ...
          // Determine route type (admin, publisher, advertiser)
          $routeType = $route['isAdmin'] ? 'admin' : ($route['isPublisher'] ? 'publisher' : 'advertiser');

          // Authentication check
          if (isset($this->authCheckCallbacks[$routeType]) && !call_user_func($this->authCheckCallbacks[$routeType])) {
